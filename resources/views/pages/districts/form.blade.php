                        ...
                        <div class="card-body">
                            ...
                            <div class="form-row">
                                <div class="form-group col-md-6">
                                    <label for="districts_name">District Name</label>
                                    <input @if(Request::segment(2)=='view' ) readonly @endif
                                        type="text"
                                        class="form-control" name="districts_name" @if(isset($element))
                                        value="{{ old('districts_name', $element->districts_name) }}" @else value="{{ old('districts_name') }}" @endif
                                        placeholder="District Code">
                                </div>
                                <div class="form-group col-md-6">
                                    <label for="districts_code">District Code</label>
                                    <input @if(Request::segment(2)=='view' ) readonly @endif
                                        type="text"
                                        class="form-control" name="districts_code" @if(isset($element))
                                        value="{{ old('districts_code', $element->districts_code) }}" @else
                                        value="{{ old('districts_code') }}" @endif placeholder="District Code">
                                </div>
                                <div class="form-group col-md-6">
                                        <label for="province_id">Province</label>
                                        <select id="province_id" @if(Request::segment(2)=='view') disabled @endif class="form-control select2"
                                            name="province_id">
                                           @foreach ($Provinces as $item)
                                            <option value="{{$item->id}}" @if(isset($element))
                                                @if(old('province_id',$element->province_id)==$item->id)
                                                selected="selected"
                                                @endif @endif
